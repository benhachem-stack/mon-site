...
(function initForm() {
  ...
  form.addEventListener('submit', e => {
    ...
    /* ─ Intégration Formspree (remplacez VOTRE_ID par votre endpoint) ─
       Pour activer l'envoi réel :
       1. Créez un compte sur formspree.io
       2. Remplacez 'VOTRE_ID' par votre form ID
       3. Décommentez le bloc fetch ci-dessous et supprimez le setTimeout
    ...
    fetch('https://formspree.io/f/VOTRE_ID', {
      ...
      if (r.ok) { form.reset(); ok.classList.add('visible'); setTimeout(() => ok.classList.remove('visible'), 5000); }
    });
    */

    /* Simulation (retirer quand Formspree est activé) */
    const btn = form.querySelector('.btn');
    btn.disabled = true;
    btn.querySelector('.btn-txt').textContent = 'Envoi en cours…';
    setTimeout(() => {
      form.reset();
      btn.disabled = false;
      btn.querySelector('.btn-txt').textContent = 'Envoyer le message';
